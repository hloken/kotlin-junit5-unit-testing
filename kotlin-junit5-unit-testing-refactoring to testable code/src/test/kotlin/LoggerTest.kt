...
import com.knowledgespike.stackunderflow.*
...
class LoggerTest {
    ...
    inner class `on construction` {
        ...
        @Test
        fun `the logBaseName should be set`() {
    ...
    inner class getMaxFileName {
        ...
        fun `should return 0 when there are no files`() {
            val logFileBaseName = ""
            val logFilesInDirectory = listOf<File>()
            val logger = Logger(configuration, paths, files)
        ...
        fun `should return 1 when there is one file`() {
            val logFilesInDirectory = listOf<File>(IoFile(java.io.File("${baseName}.log")))
        ...
        @Test
        fun `should increment when there are files with many suffixes`() {
            val logFilesInDirectory = listOf<File>(
                IoFile(java.io.File("${baseName}.log")),
    ...
    @Nested
    inner class getFilesInDirectory {
        val baseName = "baseName"
        ...
        fun `should return a list if there are files`() {
            ...
            val logger = Logger(configuration, paths, files)
            var files = logger.getAllFilesInDirectory(path)
    ...
    inner class createLog {
        ...
        fun `should create a file with the correct date`() {
            ...
            val expectedFileName = "${directoryName}\\baseName_20190807.log"
